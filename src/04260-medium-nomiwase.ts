...
type T1 = AllCombinations<'ABCD'>
type T2 = Exclude<S1, 'd'>
type T3<T> = {[K in T as string]: K}
type T4 = T3<S1>
// type StringToUnion<S> = S extends `${infer F}${infer R}`
//     ? F | StringToUnion<R>
//     : S
// type S1 = StringToUnion<'aaaaab'>
// type AllCombinations<
// S extends string,
// T extends string = StringToUnion<S>
// > = [S] extends [never]
//     ? ''
//     : {[K in T]: `${K}${AllCombinations}`}[T]
